...
def batch_gather_point(inp,idx):
    '''
input:
...
returns:
    ...
    '''
    return sampling_module.batch_gather_point(inp,idx)
...
@tf.RegisterGradient('BatchGatherPoint')
def _batch_gather_point_grad(op,out_g):
    inp=op.inputs[0]
    idx=op.inputs[1]
    return [sampling_module.batch_gather_point_grad(inp,idx,out_g),None]
...
if __name__=='__main__':
    ...
    points = np.random.randint(2, size=(1, 5, 4)).astype('float32')
    index = np.random.randint(5, size=(1, 5, 2)).astype('int32')
    #index1 = np.random.randint(5, size=(1, 2)).astype('int32')
    with tf.device('/gpu:1'):
        inp = tf.constant(points)
        idx = tf.constant(index)
        inp_ten = tf.placeholder(tf.float32, shape=(4, None, 4))
        idx_ten = tf.placeholder(tf.int32, shape=(4, None, 2))
        gather = batch_gather_point(inp_ten, idx_ten)
        print(gather)
        #idx1 = tf.constant(index1)
    with tf.Session() as sess:
        print(points, index)
        print(sess.run(batch_gather_point(inp, idx)))
