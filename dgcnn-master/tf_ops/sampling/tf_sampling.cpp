...
REGISTER_OP("BatchGatherPoint")
  .Input("inp: float32")
  .Input("idx: int32")
  .Output("out: float32")
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
    ::tensorflow::shape_inference::ShapeHandle dims1; // batch_size * npoints * 3(c)
    c->WithRank(c->input(0), 3, &dims1);
    ::tensorflow::shape_inference::ShapeHandle dims2; // batch_size * npoints * k
    c->WithRank(c->input(1), 3, &dims2);
    // batch_size * npoints * k * 3
    ::tensorflow::shape_inference::ShapeHandle output = c->MakeShape({c->Dim(dims1, 0), c->Dim(dims2, 1), c->Dim(dims2, 2), c->Dim(dims1, 2)});
    c->set_output(0, output);
    return Status::OK();
  });
...
REGISTER_OP("BatchGatherPointGrad")
  .Input("inp: float32")
  .Input("idx: int32")
  .Input("out_g: float32")
  .Output("inp_g: float32")
...
class BatchGatherPointGpuOp: public OpKernel{
  public:
    explicit BatchGatherPointGpuOp(OpKernelConstruction * context):OpKernel(context){}
    void Compute(OpKernelContext * context)override{
      const Tensor& inp_tensor=context->input(0);
      OP_REQUIRES(context,inp_tensor.dims()==3,errors::InvalidArgument("BatchGatherPoint expects (batch_size,num_points,c) inp shape"));
      int b=inp_tensor.shape().dim_size(0);
      int n=inp_tensor.shape().dim_size(1);
      int f = inp_tensor.shape().dim_size(2);
      const Tensor& idx_tensor=context->input(1);
      OP_REQUIRES(context,idx_tensor.dims()==3 && idx_tensor.shape().dim_size(0)==b,errors::InvalidArgument("BatchGatherPoint expects (batch_size,num_points, num_result) idx shape"));
      ...
      const int * idx=&(idx_flat(0));
      Tensor * out_tensor=NULL;
      OP_REQUIRES_OK(context,context->allocate_output(0,TensorShape{b,n,k,f},&out_tensor));
      auto out_flat=out_tensor->flat<float>();
      float * out=&(out_flat(0));
      batchgatherpointLauncher(b,n,k,f,inp,idx,out);
    }
};
REGISTER_KERNEL_BUILDER(Name("BatchGatherPoint").Device(DEVICE_GPU),BatchGatherPointGpuOp);
...
class BatchGatherPointGradGpuOp: public OpKernel{
  public:
    explicit BatchGatherPointGradGpuOp(OpKernelConstruction * context):OpKernel(context){}
    void Compute(OpKernelContext * context)override{
      const Tensor& inp_tensor=context->input(0);
      OP_REQUIRES(context,inp_tensor.dims()==3,errors::InvalidArgument("BatchGatherPointGradGpuOp expects (batch_size,num_points,f) inp"));
      int b=inp_tensor.shape().dim_size(0);
      int n=inp_tensor.shape().dim_size(1);
      int f=inp_tensor.shape().dim_size(2);
      const Tensor& idx_tensor=context->input(1);
      OP_REQUIRES(context,idx_tensor.dims()==3 && idx_tensor.shape().dim_size(0)==b,errors::InvalidArgument("BatchGatherPointGradGpuOp expects (batch_size,num_points, num_result) idx shape"));
      int k=idx_tensor.shape().dim_size(2);
      auto inp_flat=inp_tensor.flat<float>();
      const float * inp=&(inp_flat(0));
      auto idx_flat=idx_tensor.flat<int>();
      const int * idx=&(idx_flat(0));
      const Tensor& out_g_tensor=context->input(2);
      OP_REQUIRES(context,out_g_tensor.dims()==4 && out_g_tensor.shape().dim_size(0)==b && out_g_tensor.shape().dim_size(1)==n && out_g_tensor.shape().dim_size(2)==k && out_g_tensor.shape().dim_size(3)==f,errors::InvalidArgument("GatherPointGradGpuOp expects (batch_size,num_points,num_result,f) out_g shape"));
      auto out_g_flat=out_g_tensor.flat<float>();
      const float * out_g=&(out_g_flat(0));
      Tensor * inp_g_tensor=NULL;
      OP_REQUIRES_OK(context,context->allocate_output(0,TensorShape{b,n,f},&inp_g_tensor));
      auto inp_g_flat=inp_g_tensor->flat<float>();
      float * inp_g=&(inp_g_flat(0));
      cudaMemset(inp_g,0,b*n*f*4);
      batchscatteraddpointLauncher(b,n,k,f,out_g,idx,inp_g);
    }
};
REGISTER_KERNEL_BUILDER(Name("BatchGatherPointGrad").Device(DEVICE_GPU),BatchGatherPointGradGpuOp);
