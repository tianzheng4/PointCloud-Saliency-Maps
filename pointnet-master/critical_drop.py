...
class RandomDrop():
    def __init__(self, num_drop, num_steps):
        ...
        # simple model
        self.pred, self.end_points, self.critical_idx = MODEL.get_model_and_critical_points(self.pointclouds_pl, self.is_training_pl)
    ...
    def drop_points(self, pointclouds_pl, labels_pl, sess):
        ...
        for i in range(self.k):
            drop_indice = sess.run(self.critical_idx, feed_dict={self.pointclouds_pl: pointclouds_pl_adv,
            ...
            for j in range(pointclouds_pl.shape[0]):
                squeeze_idx = []
                count_idx = np.zeros((pointclouds_pl_adv.shape[1], ), dtype=np.int32)
                for idx in range(drop_indice.shape[1]):
                    count_idx[drop_indice[j][idx]] += 1
                for idx in range(pointclouds_pl_adv.shape[1]):
                    if count_idx[idx] != 0:
                        squeeze_idx.append(idx)
            
                #drop_indice_j = np.random.choice(np.array(squeeze_idx), self.a, replace=False)
                drop_indice_j = np.argpartition(count_idx, kth=pointclouds_pl_adv.shape[1]-self.a)[-self.a:]
                
                tmp[j] = np.delete(pointclouds_pl_adv[j], drop_indice_j, axis=0) # along N points to delete
                
            pointclouds_pl_adv = tmp.copy()
        print(pointclouds_pl_adv.shape)    
